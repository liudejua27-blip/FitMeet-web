...
- `social-agent-chat.service.ts`：核心编排逻辑，仍然偏大，是下一轮重构重点。
...
- `backend/src/agent-gateway/social-agent-chat.service.ts` 仍然过大，下一轮应继续按 planner、tool runtime、memory、task action、response composer 拆分。
...
- `fitmeet-landing` 还没有真实测试；当前 no-op test 只是保持 CI 基线一致。
- App 的真实后端端到端测试依赖可用测试账号、对象存储和后端服务，需要在 staging 环境补完整链路。
