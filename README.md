...
`social-agent-chat.service.ts` 已从早期的千行级 service 收敛到约 220 行的 controller-facing facade；后续改动应优先给新拆出的服务补独立单测，并继续压低 route turn 与 controller action 的耦合。
...
- `backend/src/agent-gateway/social-agent-chat.service.ts` 已完成 run、route turn、queued run、session query、card action、replan facade、candidate command、initial search queue 多轮抽取；下一轮应给新拆出的服务补独立单测，并继续收敛 route turn 的复杂度。
