...
const TOOL_DEFINITIONS: FitMeetAgentToolDefinition[] = [
  ...
  },
  {
    name: 'list_friends',
    description:
      '查看当前用户自己的好友、关注、可继续会话、公开约练报名来源和进行中的约练关系摘要。',
    category: FitMeetAgentToolCategory.Friend,
    permission: 'read_only',
    riskLevel: AgentActionRiskLevel.Low,
    requiresApproval: false,
          ...
          items: objectSchema(
            {
              ...
              messagesHref: { type: ['string', 'null'] },
              fromPublicIntentApplication: { type: 'boolean' },
              activeMeetLoop: { type: ['object', 'null'] },
              nextBestAction: { type: 'string' },
            },
            ['userId', 'displayName', 'nextBestAction'],
            true,
          ),
        },
        total: { type: 'integer' },
  ...
  },
  {
    ...
    inputSchema: objectSchema(
      {
        ...
        reason: { type: 'string' },
        proposalId: { type: ['string', 'integer'] },
        confirmed: { type: 'boolean' },
        tags: stringArraySchema,
      },
      ['memoryKey', 'value', 'proposalId', 'confirmed'],
    ),
    outputSchema: objectSchema(
      {
        success: { type: 'boolean' },
        status: { type: 'string' },
        confirmationRequired: { type: 'boolean' },
        proposalId: { type: ['string', 'integer', 'null'] },
        memoryKey: { type: 'string' },
        updatedFields: stringArraySchema,
        storedIn: { type: 'string' },
      },
      ['success', 'status', 'confirmationRequired'],
      true,
    ),
    permissionMode: [...CONFIRM_AND_AUTO],
    permissionAction: SocialAgentAction.GenerateContent,
    ...
    failureFallback:
      'Return a memory proposal card and wait for the user to confirm before writing long-term memory.',
  },
  {
    ...
    outputSchema: objectSchema(
      {
        ...
        adjustments: { type: 'array', items: objectSchema({}, [], true) },
        memorySignalsUsed: objectSchema({}, [], true),
        source: { type: 'string' },
      },
      ['rankedCandidateIds'],
      true,
    ...
    sideEffects: [],
    failureFallback:
      'Keep the original candidate order and explain that memory-based reranking was skipped.',
  },
  {
    ...
    outputSchema: objectSchema(
      {
        ...
        level: { type: 'string' },
        reasons: stringArraySchema,
        requiredConfirmations: stringArraySchema,
        redactedPayload: objectSchema({}, [], true),
      },
      ['allowed'],
      true,
    ),
    permissionMode: [...ALL_PERMISSION_MODES],
    executorToolName: 'check_safety_policy',
    runtimeStatus: 'implemented',
    plannerEnabled: true,
  ...
  },
  {
    ...
    permissionMode: [...ALL_PERMISSION_MODES],
    executorToolName: 'redact_sensitive_output',
    runtimeStatus: 'implemented',
    plannerEnabled: true,
