...
export function buildFriendDraftCard(input: {
  ...
  return {
    ...
    title,
    body: '可以发布到发现，也可以不公开继续私密匹配；不会自动联系任何人。',
    status: 'waiting_confirmation',
    data: {
      taskId: input.taskId,
      socialRequestId: input.draft.socialRequestId,
      ...
      locationText: input.slots.locationText ?? null,
      ...
      bodyPreference: input.slots.bodyPreference ?? null,
      appearancePreference: input.slots.appearancePreference ?? null,
