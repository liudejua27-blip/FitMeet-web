...
export class SocialAgentBrainService {
  ...
  private isProfileEnrichmentRequest(message: string): boolean {
    return /((帮我|请|可以|想|需要|继续)?.{0,12}(完善|补充|补全|整理|更新).{0,16}(画像|资料|个人信息|信息|偏好))|((资料|个人信息|信息|画像).{0,12}(还缺|还差|缺什么|缺哪些|完善|补充|补全))|(问我.{0,8}(几个)?问题)|(调用工具.*画像|写入.*画像|保存.*画像|存到.*画像|把刚才.*画像|把上面.*画像)/i.test(
      message,
    );
  }
