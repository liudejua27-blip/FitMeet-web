...
type SocialMatchDynamicExplanation = {
...
export class SocialAgentCandidatePoolService {
  ...
  private candidateBase(input: {
    ...
    const dynamicExplanation = this.buildSocialMatchDynamicExplanation({
      displayName: input.displayName,
      city: input.city,
      interestTags: input.interestTags,
      commonTags: input.commonTags,
  ...
  private buildSocialMatchDynamicExplanation(input: {
    displayName: string;
    city: string;
    interestTags: string[];
    commonTags: string[];
    ...
    const matchPoints = this.uniqueStrings([
    ...
    const boundaryNotes = this.uniqueStrings([
      safety?.publicPlaceOnly
        ? '第一次建议选择校园操场、公园或其他公共场所。'
        : '',
      safety?.locationSharingAllowed === false
        ? '不建议直接共享精确位置。'
        : '',
      guidance?.shouldAvoidNight ||
      behavior?.nightBoundary === 'avoids_late_private'
    ...
    const dynamicSignalReasons = this.uniqueStrings([
      behavior?.summary,
      ...(behavior?.insights ?? []),
    ...
    const whyNow = this.socialMatchWhyNow(input, behaviorScore);
    ...
    return {
      ...
      continuousFilterHints: this.uniqueStrings([
