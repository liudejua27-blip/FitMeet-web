...
function timelineMessageFromEvent(
  ...
  if (eventType === AgentTaskEventType.SocialAgentMessageAssistant) {
    ...
    const activityResults = readActivityResults(payload.activityResults);
    return {
      id,
      role: 'assistant',
  ...
  if (eventType === AgentTaskEventType.SocialAgentCandidatesReturned) {
    const candidates = readTimelineCandidates(task, payload.candidates);
    const activityResults = readActivityResults(payload.activityResults);
    const text =
  ...
  const rawDraft = isRecord(chatRun.socialRequestDraft)
    ...
    : isRecord(chat.socialRequestDraft)
      ...
      : isRecord(eventResult?.socialRequestDraft)
        ...
        : null;
  const storedCandidates = readSocialAgentStoredCandidateSummaries(task)
    .map((candidate) => candidateFromStoredSummary(task, candidate))
...
function readCandidateResultFromEvents(
  ...
  const event = [...events]
    ...
    .find(
      (item) =>
        normalizeAgentTaskEventType(item.eventType) ===
        AgentTaskEventType.SocialAgentCandidatesReturned,
    );
  if (!event || !isRecord(event.payload)) return null;
  const payload = event.payload;
  return {
    candidates: readTimelineCandidates(task, payload.candidates),
...
function readTimelineCandidates(
  task: AgentTask,
  value: unknown,
): SocialAgentChatCandidate[] {
...
function readActivityResults(value: unknown): SocialAgentActivityResult[] {
  if (!Array.isArray(value)) return [];
  return value
    .filter((item): item is Record<string, unknown> => isRecord(item))
    .map((item) => {
