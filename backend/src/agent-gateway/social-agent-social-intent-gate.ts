...
const profileEnrichmentRequestPattern =
  /((帮我|请|可以|想|需要|继续)?.{0,12}(完善|补充|补全|整理|更新|修改|编辑).{0,16}(画像|资料|个人信息|信息|偏好))|((资料|个人信息|信息|画像).{0,12}(还缺|还差|缺什么|缺哪些|完善|补充|补全|更新|修改|编辑))|(问我.{0,8}(几个)?问题)|(把刚才.*写入画像|保存到.*画像|调用工具.*画像|工具.*完善.*画像|写入.*画像|存到.*画像|人物画像|ai画像|AI画像)/i;
...
export function isProfileEnrichmentDominant(message: string): boolean {
