...
export class SocialAgentTaskMemoryStateMachineService {
  ...
  private extractTimeWindow(text: string): string | null {
    const matches =
      text.match(
        /(\d{1,2}[./月]\d{1,2}日?\s*(?:今天|明天|后天|周[一二三四五六日天]|周末)?\s*(?:早上|上午|中午|下午|晚上|今晚)?\s*(?:\d{1,2}|[一二两三四五六七八九十]+)(?::\d{2})?\s*点?(?:半|左右)?)/,
      ) ??
      text.match(
        /((?:(?:今天|明天|后天|周[一二三四五六日天]|周末)\s*)?(?:早上|上午|中午|下午|晚上|今晚)\s*(?:\d{1,2}|[一二两三四五六七八九十]+)(?::\d{2})?\s*点(?:半|左右)?|(?:今天|明天|后天|周[一二三四五六日天]|周末)\s*(?:\d{1,2}|[一二两三四五六七八九十]+)(?::\d{2})?\s*点(?:半|左右)?|\d{1,2}(?::\d{2})?\s*点(?:半|左右)?|[二两三四五六七八九十]+(?::\d{2})?\s*点(?:半|左右)?)/,
      ) ??
  ...
  private extractLocation(
    ...
    const named =
      text.match(
        /((?:崂山区|市南区|市北区|李沧区|黄岛区)[\u4e00-\u9fa5A-Za-z0-9·-]{0,16}(?:公园|广场|体育馆|健身房|大学|校区|商场|书店|咖啡店|中心)(?:附近|周边)?)/,
      ) ??
      text.match(
        /((?:青岛|北京|上海|深圳|广州|杭州|南京|成都|武汉|西安|重庆|苏州|厦门|天津|长沙|郑州|济南|宁波)[\u4e00-\u9fa5A-Za-z0-9·-]{0,16}(?:公园|广场|体育馆|健身房|大学|校区|商场|书店|咖啡店|中心)(?:附近|周边)?)/,
      ) ??
      text.match(
        /((?:崂山区|市南区|市北区|李沧区|黄岛区|青岛大学|五四广场|中山公园|奥帆中心|大学城|附近)[\u4e00-\u9fa5A-Za-z0-9·-]{0,12})/,
      );
    const rawValue =
      named?.[1] ??
      text.match(
