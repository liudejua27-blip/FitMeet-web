...
export class TravelUnderstandingService {
  ...
  slotsFromUnderstanding(
    ...
    return {
      destination: this.text(understanding.destination) || undefined,
  ...
  private prompt(input: {
    ...
    return JSON.stringify({
      instruction:
        'Extract travel-companion loop intent and slots from the user message. Return only JSON. Do not publish, match, send messages, save profile, or book anything. Only identify the user-facing travel companion card fields.',
      allowedIntentValues: [
        'travel',
        'workout',
        'friend',
