...
- Runtime contract: Social Codex Runtime visible process events, durable
  thread/task/run state, governed Life Graph memory, and approval-first social
  side effects.
...
| Production token/cost evidence | `REQUIRE_AGENT_COST_DATA=true MIN_WORKFLOW_ROUTE_RATE=0.30 MIN_CACHE_HIT_RATE=0.30 MIN_PROMPT_PREFIX_REUSE_RATE=0.70 MIN_STAGE_PROMPT_PREFIX_REUSE_RATE=0.70 MAX_PROMPT_PREFIX_DISTINCT_RATIO=0.30 MAX_STAGE_PROMPT_PREFIX_DISTINCT_RATIO=0.30 MAX_AVG_LLM_CALLS_PER_RUN=3 AGENT_TOKEN_COST_EVIDENCE_FILE=artifacts/agent-release-evidence/token-cost.json FITMEET_ADMIN_JWT=<admin-jwt> scripts/verify-agent-token-cost.sh` | Stores `fitmeet.agent-token-cost-evidence.v1` JSON with avg LLM calls/run, prompt/cached tokens, cache hit rate, workflow route rate, stage costs, prompt prefix reuse, and prompt prefix hash drift. The same thresholds are archived into the JSON evidence and rechecked by `launch-status.sh`. |
| Distributed Agent cache       | `SOCIAL_AGENT_CACHE_BACKEND=redis SOCIAL_AGENT_TOOL_RESULT_CACHE_BACKEND=redis scripts/ecs-host-preflight.sh`                                                                 | ECS production cache preflight fails if Agent exact LLM output, semantic response, embedding, and tool-result caches would fall back to process-local storage across backend/worker replicas. |
...
| Strict Agent release matrix  | `scripts/agent-release-matrix.sh --strict-agent-smoke --build`                                                                                                             | Builds both apps and requires remote readiness, 20-turn memory, empty-candidate, and SSE abort smokes before a release candidate. |
| Strict full Agent matrix     | `AGENT_SMOKE_ALLOW_MUTATIONS=true scripts/agent-release-matrix.sh --strict-agent-full-smoke --build`                                                                       | Full mutating opportunity smoke plus 20-turn memory, empty-candidate, and SSE abort. Dedicated smoke users only. |
| Opportunity smoke report     | `AGENT_SMOKE_REPORT_FILE=artifacts/agent-release-evidence/opportunity-smoke.json RUN_AGENT_OPPORTUNITY_SMOKE=true bash scripts/verify-agent-release.sh`                    | Writes `fitmeet.agent-opportunity-smoke-report.v1` JSON with passed Social Codex milestones, scenario knobs, check count, and failure reason for release evidence. |
...
| Final Agent cutover status   | `REQUIRE_AGENT_REMOTE_SMOKE_EVIDENCE=true AGENT_REMOTE_SMOKE_EVIDENCE_FILE=<evidence.md> REQUIRE_AGENT_TOKEN_COST_EVIDENCE=true AGENT_TOKEN_COST_EVIDENCE_FILE=<token-cost.json> scripts/launch-status.sh --topology ecs --skip-ios-testflight-check` | Launch status fails unless the redacted ECS evidence proves readiness, 20-turn memory, empty-candidate recovery, full opportunity, SSE abort, and zero-exit post-deploy smoke. It also rechecks archived token/cost thresholds for workflow route rate, cache hit rate, prompt prefix reuse, per-stage prompt prefix reuse, prompt prefix hash drift, and max avg LLM calls/run. |
...
| 20-turn social memory continuity              | Optional real API smoke requires one active task, stable completed slots, no repeated time/place/activity questions, and no hidden thread restart through a 20-turn continuation.     |
...
| Recommendation signals feed ranking          | Backend release specs include `social-agent-user-interest-event.service.spec.ts` and `social-agent-candidate-pool.service.spec.ts`; frontend release specs include `DiscoverClosure.test.ts` and `UserProfileInterestSignals.test.ts` so Discover clicks, public intent detail dwell, profile views, candidate saves/skips/invites, and recency decay remain wired into candidate ranking. |
...
- `scripts/ecs-post-deploy-smoke.sh` automatically invokes the same preflight
  before opportunity readiness, 20-turn memory continuity, empty-candidate
  recovery, full opportunity, and SSE abort smoke.
- `scripts/verify-agent-release.sh` also invokes the preflight before optional
  real API opportunity, 20-turn memory, empty-candidate, or SSE abort smoke.
...
- 20-turn memory smoke is opt-in with `RUN_AGENT_20_TURN_MEMORY_SMOKE=true`,
  `RUN_AGENT_SKILL_EVAL_API=20-turn-memory`, or
  `scripts/ecs-post-deploy-smoke.sh --run-agent-20-turn-memory-smoke`. It should
  run after memory, context, planner, route, or fallback changes and uses a
  dedicated smoke account because it writes chat/search smoke state.
...
# Prove long social task memory does not forget or repeat questions.
scripts/agent-release-matrix.sh --skip-browser-qa --20-turn-memory-smoke
...
# Run the same 20-turn proof directly from the skill eval runner.
...
5. `launch-status.sh` passes with
   ...
   `REQUIRE_AGENT_TOKEN_COST_EVIDENCE=true` using the generated redacted smoke
   evidence markdown and token/cost JSON evidence. Token/cost evidence must
   include threshold values or equivalent launch-time env overrides for
   workflow route rate, cache hit rate, prompt prefix reuse, per-stage prompt
   prefix reuse, prompt prefix hash drift, and max average LLM calls per run.
