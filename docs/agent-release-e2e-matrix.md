...
| Production token/cost evidence | `REQUIRE_AGENT_COST_DATA=true AGENT_TOKEN_COST_EVIDENCE_FILE=artifacts/agent-release-evidence/token-cost.json FITMEET_ADMIN_JWT=<admin-jwt> scripts/verify-agent-token-cost.sh` | Stores `fitmeet.agent-token-cost-evidence.v1` JSON with avg LLM calls/run, prompt/cached tokens, cache hit rate, workflow route rate, stage costs, and prompt prefix reuse. |
...
| Final Agent cutover status   | `REQUIRE_AGENT_REMOTE_SMOKE_EVIDENCE=true AGENT_REMOTE_SMOKE_EVIDENCE_FILE=<evidence.md> REQUIRE_AGENT_TOKEN_COST_EVIDENCE=true AGENT_TOKEN_COST_EVIDENCE_FILE=<token-cost.json> scripts/launch-status.sh --topology ecs --skip-ios-testflight-check` | Launch status fails unless the redacted ECS evidence proves readiness, 20-turn memory, empty-candidate recovery, full opportunity, SSE abort, and zero-exit post-deploy smoke, and token/cost evidence proves live LLM cost buckets. |
...
5. `launch-status.sh` passes with
   ...
   evidence markdown and token/cost JSON evidence.
