...
- iOS local release risk has been reduced: `Scripts/release-preflight-ios.sh` now passes on the booted iPhone 17 simulator, including Release simulator build and `FitMeetAlphaTests`. iOS staging E2E still requires a deployed backend, real credentials, a second message target user, and object storage; it cannot pass in a credential-free local run.
...
- Blocked: local DB-backed migration/runtime smoke remains unavailable because `docker --version` and `docker compose version` return `command not found`, and `pnpm --dir backend migration:status` still fails against Postgres `localhost:5432/fitness_app` with `ECONNREFUSED`.
...
- Passed: landing `pnpm --dir fitmeet-landing test`; current landing baseline runs source smoke tests, `next build`, and rendered-output smoke tests rather than a no-op placeholder.
