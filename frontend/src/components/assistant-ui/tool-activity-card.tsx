...
import { CardActionDismissible, CardActionSummary } from './tool-card-actions';
...
export function ActivityOpportunityCard({ card }: { card: SchemaDrivenAssistantCard }) {
  ...
  const dismissedNotice = (
    <article
      className="w-full rounded-[18px] border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 shadow-[0_8px_24px_rgba(15,23,42,0.06)]"
      data-testid="activity-opportunity-card-dismissed"
      data-card-model="assistant-ui-opportunity-card-dismissed"
    >
      已取消发布，不会出现在发现页。
    </article>
  );
  ...
  return (
    <CardActionDismissible card={card} dismissed={dismissedNotice}>
      <article
