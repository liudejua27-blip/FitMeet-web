...
import { LocationPicker } from '../meet/LocationPicker';

interface CreatePostModalProps {
  open: boolean;
  onClose: () => void;
  onCreated?: (item: Post | Meet, type: PublishType) => void;
}

type PublishType = Exclude<ContentType, 'all'>;

const SPORTS_TAGS = ['增肌', '减脂', '跑步', '瑜伽', '游泳', '户外', '武术', '球类', 'HIIT', '深蹲', '卧推', '硬拉'];

export const CreatePostModal = memo(function CreatePostModal({ open, onClose, onCreated }: CreatePostModalProps) {
  const { containerRef, handleBackdropClick } = useModalA11y<HTMLDivElement>({ open, onClose });
  const [type, setType] = useState<PublishType>('log');
  ...
  const [selectedPlace, setSelectedPlace] = useState<AmapPlace | null>(null);
  ...
  const selectedSport = getSportNode(sport);
  ...
  const toggleLimitedTag = useCallback((tag: string, setter: Dispatch<SetStateAction<string[]>>) => {
    setter((prev) => (prev.includes(tag) ? prev.filter((item) => item !== tag) : prev.length < 3 ? [...prev, tag] : prev));
  }, []);
  ...
  const handleUseCurrentLocation = useCallback(async () => {
    ...
    setLocationError('');
    try {
      handlePlaceSelect(await resolveCurrentPlace(await getBrowserLocation()));
    } catch (error) {
      setLocationError(error instanceof Error ? error.message : '定位失败，请检查浏览器权限');
    } finally {
      setLocating(false);
    }
  }, [handlePlaceSelect]);
  ...
  const handleImageUpload = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (!files?.length || images.length >= 9) return;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!selectedPlace) {
      setLocationError('请选择地点或使用当前位置后再发布');
      return;
    }
    setSubmitted(true);
    setSubmitError('');

    const isCustomSport = sport === 'other';
    const customName = customCategoryName.trim();
    const apiSport = isCustomSport ? 'other' : sport;
    const tags = buildPostTags({
      customCategoryName: isCustomSport ? customName : '',
      equipmentTags,
      scenarioTags,
      selectedTags,
      subcategoryId,
    ...
    try {
      if (type === 'meet') {
        const created = await dataService.createMeet({
          title,
          type: apiSport,
          sport: isCustomSport && customName ? customName : getSportLabel(apiSport),
          desc: content,
          time: meetTime,
          loc: selectedPlace.name,
          address: [selectedPlace.district, selectedPlace.address].filter(Boolean).join(' '),
          poiId: selectedPlace.id,
          lat: selectedPlace.location.lat,
          lng: selectedPlace.location.lng,
          city: selectedPlace.district || '当前位置',
          maxSlots: parseInt(meetSlots, 10) || 4,
          price: '免费',
          level: 'all',
        } as Partial<Meet>);
        onCreated?.(created, type);
      } else {
