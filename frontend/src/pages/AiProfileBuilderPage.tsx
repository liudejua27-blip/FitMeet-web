...
export function AiProfileBuilderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0b] text-[#f6efe5]">
      <div className="border-b border-white/10 bg-black/30 px-4 py-5 backdrop-blur sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-[#c9c0b4]">
              通过对话把性格、兴趣、边界和匹配偏好整理成结构化人物卡。
            </p>
          </div>
          <div className="min-w-[180px] rounded-lg border border-white/10 bg-white/[0.04] px-4 py-3">
            <div className="text-xs font-bold text-[#c9c0b4]">画像完整度</div>
            <div className="mt-2 flex items-end gap-2">
              <span className="text-3xl font-black text-[#c8ff80]">{completion.percent}%</span>
        ...
        <section className="space-y-5">
          ...
          {!draft ? (
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.04] p-5">
                <h3 className="text-base font-black text-white">可见性</h3>
                <div className="mt-4 rounded-lg border border-white/10 bg-black/20 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <h4 className="text-sm font-black text-white">AI match signals</h4>
                      <p className="mt-1 text-xs leading-5 text-[#b8afa2]">
                        Public tags are safe previews. Private and sensitive tags only guide matching.
                      </p>
                    </div>
                    <Link
                      to="/agent-inbox"
                  ...
                  <div className="mt-4 rounded-lg border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-xs font-bold leading-5 text-amber-100">
                    Wealth, income, appearance, status, resources, contact info, and exact address stay private in v1.
                  </div>
                </div>

                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <Toggle label="进入发现池" checked={draft.visibility.profileDiscoverable} onChange={(value) => updateVisibility('profileDiscoverable', value)} />
                  <Toggle label="允许被推荐" checked={draft.visibility.agentCanRecommendMe} onChange={(value) => updateVisibility('agentCanRecommendMe', value)} />
                  <Toggle label="确认后开聊" checked={draft.visibility.agentCanStartChatAfterApproval} onChange={(value) => updateVisibility('agentCanStartChatAfterApproval', value)} />
                </div>
                <label className="mt-4 flex items-center gap-3 rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm font-bold text-[#f6efe5]">
                  <input
                    type="checkbox"
...
function Toggle({
  ...
  return (
    <label className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm font-bold text-[#f6efe5]">
      {label}
      <input
        type="checkbox"
        checked={checked}
