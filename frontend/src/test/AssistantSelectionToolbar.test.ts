...
describe('assistant-ui selection toolbar', () => {
  it('keeps quote actions above selected text instead of covering message content', () => {
    ...
    expect(threadSource).toContain("data-offset-y=\"96\"");
    expect(threadSource).toContain("data-selection-safe-distance=\"large\"");
  ...
  it('keeps selected assistant text readable while the toolbar is visible', () => {
    ...
    expect(globalCss).toContain('--assistant-selection-toolbar-offset-y: 96px;');
    expect(globalCss).toContain('background: rgba(0, 115, 230, 0.1);');
