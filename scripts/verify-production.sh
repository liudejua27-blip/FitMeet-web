...
usage() {
  cat <<'EOF'
Usage: scripts/verify-production.sh [--base-url https://www.ourfitmeet.cn] [--api-base-url https://www.ourfitmeet.cn/api] [--agent-token token] [--expect-release-commit <sha>] [--run-public-intent-write]
...
discover_body="$(curl_status "Discover public intents" "${API_BASE_URL}/public/social-intents?page=1&limit=5" "200")"
...
node - "${health_body}" "${ready_body}" "${discover_body}" <<'NODE'
const fs = require('fs');
const health = JSON.parse(fs.readFileSync(process.argv[2], 'utf8'));
const ready = JSON.parse(fs.readFileSync(process.argv[3], 'utf8'));
const publicIntents = JSON.parse(fs.readFileSync(process.argv[4], 'utf8'));
if (health.status !== 'ok') {
...
if (!Array.isArray(publicIntents.data)) {
  console.error('Public social intents payload does not expose a data array.');
  process.exit(1);
}
NODE
ok "Health, readiness, and public-intent payload shapes are readable"
